    ...
    if (buttonArray.count>0) {//防止没创建前为空
        for (UIButton *labelView in buttonArray) {
            if (labelView.tag==index) {
                if (labelView.tag==0) {
                    //深绿线
                    label1.backgroundColor = RGBA(179, 211, 85, 1);
                }else if(labelView.tag == 1){
                    label2.backgroundColor = RGBA(179, 211, 85, 1);
                }else{
                    label3.backgroundColor = RGBA(179, 211, 85, 1);
                }
                currentSelected = labelView;
            }else{
                if (labelView.tag == 0) {
                    //绿线
                    label1.backgroundColor = RGBA(238, 238, 238, 1);
                }else if(labelView.tag == 1){
                    label2.backgroundColor = RGBA(238, 238, 238, 1);
                }else{
                    label3.backgroundColor = RGBA(238, 238, 238, 1);
                }
            }
        }
    }
...
-(void)setTitleArray:(NSArray *)titleArray
{
    ...
    for (int i = 0; i<_titleArray.count; i++) {
        ...
        [btn setTitleColor:RGBA(87, 173, 104, 1) forState:UIControlStateNormal];
        ...
        if (i == 0) {
            currentSelected=btn;
            // 深绿线
            label1=[[UILabel alloc]initWithFrame:CGRectMake(0, 45.5, ScreenWidth/3, 2.5)];
            label1.backgroundColor = RGBA(179, 211, 85, 1);
            ...
            //绿线
            label2 = [[UILabel alloc]initWithFrame:CGRectMake(width, 45.5, ScreenWidth/3, 2.5)];
            label2.backgroundColor = RGBA(238, 238, 238, 1);
            ...
            //绿线
            label3 = [[UILabel alloc]initWithFrame:CGRectMake(width * 2, 45.5, ScreenWidth/3, 2.5)];
            label3.backgroundColor = RGBA(238, 238, 238, 1);
